...
def make_image_request(query: str, results: int = 10) -> list[dict]:
    """
    makes a image search request to duckduckgo search engine and retrieves specified results
    ...
    Returns:
        list[dict]: list of dict (can be empty list) with fields - title, image and url.
    """
    try:
        results = search_client.images(query, max_results=results)
    except Exception as e:
        print("Error while searching duckduckgo client - ", e)
        results = []
